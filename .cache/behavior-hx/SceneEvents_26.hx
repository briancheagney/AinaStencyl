...
class SceneEvents_26 extends SceneScript
{
	...
	public function new(dummy:Int, dummy2:Engine)
	{
		...
		_MoonPicker = 0.0;
		nameMap.set("Falsable", "_Falsable");
		_Falsable = false;
	...
	override public function init()
	{
		...
		Engine.engine.setGameAttribute("L10Col02TotalNum", ((Engine.engine.getGameAttribute("L10Col02TotalNum") : Float) + 1));
		runLater(1000 * .1, function(timeTask:TimedTask):Void
		{
			...
			_Item[1] = (Engine.engine.getGameAttribute("L10Col02TotalNum") : Float);
			_Item[2] = (((("" + Date.now().getHours()) + (":"))) + ((((("" + Date.now().getMinutes()) + (":"))) + ("" + Date.now().getSeconds()))));
			(Engine.engine.getGameAttribute("L10Date01") : Array<Dynamic>)[Std.int((Engine.engine.getGameAttribute("L10Col02TotalNum") : Float))] = _Item;
		}, null);
		...
		_Target.setAnimation("NullClubO");
		...
		addMouseOverActorListener(_Target, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if(((Engine.engine.getGameAttribute("TargetPush") : Bool) == true))
				{
					...
					/* CardPicker is a variable that determines what card the non-match is going to be. */
					/* In this example */
					_CardPicker = randomInt(2, 2);
					if((_CardPicker == 1))
					{
						_NoMatch.setAnimation("NullHeart");
					}
					else if((_CardPicker == 2))
					{
						_NoMatch.setAnimation("NullClub");
					}
					else if((_CardPicker == 3))
					{
		...
		addMouseOverActorListener(_Match, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((_Matchable == true))
				{
					...
					_Match.setX(-1200);
					trace("True");
					trace((Engine.engine.getGameAttribute("L10Date01") : Array<Dynamic>));
					if((_counter == 10))
					{
		...
		addMouseOverActorListener(_NoMatch, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((_Matchable == true))
				{
					...
					_Match.disableActorDrawing();
					_Target.disableActorDrawing();
					_NoMatch.setAnimation("Null");
					trace("False");
					trace((Engine.engine.getGameAttribute("L09Date01") : Array<Dynamic>));
					if((_counter == 10))
					{
