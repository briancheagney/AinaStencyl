...
class SceneEvents_37 extends SceneScript
{
	...
	override public function init()
	{
		...
		Engine.engine.setGameAttribute("L16_2var_Color_TotalNum", ((Engine.engine.getGameAttribute("L16_2var_Color_TotalNum") : Float) + 1));
		runLater(1000 * .1, function(timeTask:TimedTask):Void
		{
			...
			_Item[1] = (Engine.engine.getGameAttribute("L16_2var_Color_TotalNum") : Float);
			_Item[2] = (((("" + Date.now().getHours()) + (":"))) + ((((("" + Date.now().getMinutes()) + (":"))) + ("" + Date.now().getSeconds()))));
			(Engine.engine.getGameAttribute("L16Date") : Array<Dynamic>)[Std.int((Engine.engine.getGameAttribute("L16_2var_Color_TotalNum") : Float))] = _Item;
		}, null);
		...
		addMouseOverActorListener(_Match, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((_Matchable == true))
				{
					...
					_Match.setX(-1200);
					trace("True");
					trace((Engine.engine.getGameAttribute("L16Date") : Array<Dynamic>));
					if((_counter == 10))
					{
