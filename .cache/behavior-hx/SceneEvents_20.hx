...
class SceneEvents_20 extends SceneScript
{
	...
	public function _customEvent_ChooseTarget():Void
	{
		...
		if((_TargetChooser == 1))
		{
			_Target.setAnimation("OrangeClub01");
			_Match.setAnimation("OrangeClub01");
			_NoMatch.setAnimation("BlueClub01");
			_NoMatchGreen = true;
		...
		else if((_TargetChooser == 2))
		{
			...
			_Match.setAnimation("BlueClub01");
			_NoMatch.setAnimation("OrangeClub01");
			_NoMatchRed = true;
	...
	override public function init()
	{
		...
		Engine.engine.setGameAttribute("L11ColSwTotalNum", ((Engine.engine.getGameAttribute("L11ColSwTotalNum") : Float) + 1));
		runLater(1000 * .1, function(timeTask:TimedTask):Void
		{
			...
			_Item[1] = (Engine.engine.getGameAttribute("L11ColSwTotalNum") : Float);
			_Item[2] = (((("" + Date.now().getHours()) + (":"))) + ((((("" + Date.now().getMinutes()) + (":"))) + ("" + Date.now().getSeconds()))));
			(Engine.engine.getGameAttribute("L11Date01") : Array<Dynamic>)[Std.int((Engine.engine.getGameAttribute("L11ColSwTotalNum") : Float))] = _Item;
		}, null);
			...
			if(wrapper.enabled && 3 == mouseState)
			{
				if((_Matchable == true))
				{
					...
					runLater(1000 * .01, function(timeTask:TimedTask):Void
					{
						...
						{
							/* These "NoMatchGreen" and "NoMatchRed" used to make sense with the different cards...not quite sure yet what they're doing here. */
					...
					_Match.setX(-1200);
					trace("True");
					trace((Engine.engine.getGameAttribute("L11Date01") : Array<Dynamic>));
					if((_counter == 10))
					{
