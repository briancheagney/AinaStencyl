		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if((_pushnum >= 3))
				{
					...
					{
						Engine.engine.setGameAttribute("SmallButtonSizer", true);
						_pushnum = 0;
					}
					else if(((Engine.engine.getGameAttribute("SmallButtonSizer") : Bool) == true))
					{
						Engine.engine.setGameAttribute("SmallButtonSizer", false);
						_pushnum = 0;
					}
				}
			}
