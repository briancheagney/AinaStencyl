...
class SceneEvents_48 extends SceneScript
{
	...
	override public function init()
	{
		...
		Engine.engine.setGameAttribute("L14Num02TotalNum", ((Engine.engine.getGameAttribute("L14Num02TotalNum") : Float) + 1));
		runLater(1000 * .1, function(timeTask:TimedTask):Void
		{
			...
			_Item[1] = (Engine.engine.getGameAttribute("L14Num02TotalNum") : Float);
			_Item[2] = (((("" + Date.now().getHours()) + (":"))) + ((((("" + Date.now().getMinutes()) + (":"))) + ("" + Date.now().getSeconds()))));
			(Engine.engine.getGameAttribute("L14Date") : Array<Dynamic>)[Std.int((Engine.engine.getGameAttribute("L14Num02TotalNum") : Float))] = _Item;
		}, null);
		...
		addMouseOverActorListener(_Match, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((_Matchable == true))
				{
					...
					_Match.setX(-1200);
					trace("True");
					trace((Engine.engine.getGameAttribute("L14Date") : Array<Dynamic>));
					if((_counter == 10))
					{
					...
					runLater(1000 * .01, function(timeTask:TimedTask):Void
					{
						runLater(1000 * .5, function(timeTask:TimedTask):Void
						{
							...
							else if((_NoMatchChooser == 2))
							{
								...
								{
									getLastCreatedActor().setAnimation("BlackClub01");
									_NoMatch.setX(((getScreenWidth() / 2) - (((_NoMatch.getWidth()) / 1) + ((_NoMatch.getWidth()) / 4))));
									_NoMatch.setY(_Matchyvalue);
									_NoMatch.disableActorDrawing();
								}
