...
class SceneEvents_35 extends SceneScript
{
	...
	public var _NoMatchMoon:Bool;
	public var _NoMatchClub:Bool;
	...
	public function _customEvent_ChooseTarget():Void
	{
		...
		/* Here I'm going to switch the clubs and moons to hearts and pauses. */
		if((_TargetChooser == 1))
		{
			_Target.setAnimation("NullHeart");
			_Match.setAnimation("NullHeart");
			_NoMatch.setAnimation("NullDiamond");
			_NoMatchMoon = true;
			_ClubPicker = (_ClubPicker + 1);
		}
		else if((_TargetChooser == 2))
		{
			_Target.setAnimation("NullDiamond");
			_Match.setAnimation("NullDiamond");
			_NoMatch.setAnimation("NullHeart");
			_NoMatchClub = true;
		...
		_customEvent_ChooseTarget2();
	}
	
	/* ========================= Custom Event ========================= */
	public function _customEvent_ChooseTarget2():Void
	{
		trace("fade in now.");
	...
	public function new(dummy:Int, dummy2:Engine)
	{
		...
		_MoonPicker = 0.0;
		nameMap.set("NoMatchMoon", "_NoMatchMoon");
		_NoMatchMoon = false;
		nameMap.set("NoMatchClub", "_NoMatchClub");
		_NoMatchClub = false;
	...
	override public function init()
	{
		...
		Engine.engine.setGameAttribute("L08TotalNum", ((Engine.engine.getGameAttribute("L08TotalNum") : Float) + 1));
		runLater(1000 * .1, function(timeTask:TimedTask):Void
		{
			...
			_Item[1] = (Engine.engine.getGameAttribute("L08TotalNum") : Float);
			_Item[2] = (((("" + Date.now().getHours()) + (":"))) + ((((("" + Date.now().getMinutes()) + (":"))) + ("" + Date.now().getSeconds()))));
			(Engine.engine.getGameAttribute("L08Date01") : Array<Dynamic>)[Std.int((Engine.engine.getGameAttribute("L08TotalNum") : Float))] = _Item;
		}, null);
		...
		/* Create the target */
		createRecycledActor(getActorType(245), (Engine.engine.getGameAttribute("TargetX") : Float), (Engine.engine.getGameAttribute("TargetY") : Float), Script.FRONT);
		_Target = getLastCreatedActor();
		_Target.setAnimation("NullHeart");
		_Target.disableActorDrawing();
		/* create the NONmatch */
		createRecycledActor(getActorType(245), (Engine.engine.getGameAttribute("MatchX") : Float), (Engine.engine.getGameAttribute("MatchY") : Float), Script.FRONT);
		...
		/* I don't know what these two things are below here.
 */
		_Item = new Array<Dynamic>();
		...
		runLater(1000 * 1, function(timeTask:TimedTask):Void
		{
			if(wrapper.enabled)
			{
				...
				trace((("Card 01 X = ") + ("" + Engine.engine.getGameAttribute("MatchX01"))));
				trace((("Card 02 X = ") + ("" + Engine.engine.getGameAttribute("MatchX02"))));
			}
		}, null);
			...
			if(wrapper.enabled && 3 == mouseState)
			{
				if((_Matchable == true))
				{
					...
					runLater(1000 * .01, function(timeTask:TimedTask):Void
					{
						...
						{
							if((_NoMatchClub == true))
							{
								if((_LeftRightPicker == 1))
								{
									_Match.setX(((getScreenWidth() / 2) - (((_Match.getWidth()) / 1) + ((_Match.getWidth()) / 4))));
								...
								{
									_Match.setX(((getScreenWidth() / 2) + ((_Match.getWidth()) / 4)));
									_Match.setY(_Matchyvalue);
									_Match.setAnimation("NullDiamond");
									_Match.disableActorDrawing();
								}
