...
class SceneEvents_15 extends SceneScript
{
	...
	public function _customEvent_ChooseTarget():Void
	{
		...
		if((_TargetChooser == 1))
		{
			...
			_NoMatch.setAnimation("NullMoon");
			_NoMatchMoon = true;
			_ClubPicker = (_ClubPicker + 1);
		}
		else if((_TargetChooser == 2))
		{
			_Target.setAnimation("NullMoon");
			_Match.setAnimation("NullMoon");
			_NoMatch.setAnimation("NullClub");
			_NoMatchClub = true;
	...
	override public function init()
	{
		...
		Engine.engine.setGameAttribute("L07TotalNum", ((Engine.engine.getGameAttribute("L07TotalNum") : Float) + 1));
		runLater(1000 * .1, function(timeTask:TimedTask):Void
		{
			...
			_Item[1] = (Engine.engine.getGameAttribute("L07TotalNum") : Float);
			_Item[2] = (((("" + Date.now().getHours()) + (":"))) + ((((("" + Date.now().getMinutes()) + (":"))) + ("" + Date.now().getSeconds()))));
			(Engine.engine.getGameAttribute("L07Date01") : Array<Dynamic>)[Std.int((Engine.engine.getGameAttribute("L07TotalNum") : Float))] = _Item;
		}, null);
			...
			if(wrapper.enabled)
			{
				if(_Target.isMousePressed())
				{
					if(((Engine.engine.getGameAttribute("TargetPush") : Bool) == true))
					{
						...
						_Target.setFilter([createTintFilter(Utils.getColorRGB(0,0,0), 70/100)]);
						if((_LeftyCount > 4))
						{
							_LeftRightPicker = 2;
						}
		...
		addMouseOverActorListener(_Match, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((_Matchable == true))
				{
					...
					_Match.setX(-1200);
					trace("True");
					trace((Engine.engine.getGameAttribute("L07Date01") : Array<Dynamic>));
					if((_counter == 10))
					{
