...
class SceneEvents_38 extends SceneScript
{
	...
	override public function init()
	{
		...
		Engine.engine.setGameAttribute("L15NumSwTotalNum", ((Engine.engine.getGameAttribute("L15NumSwTotalNum") : Float) + 1));
		runLater(1000 * .1, function(timeTask:TimedTask):Void
		{
			...
			_Item[1] = (Engine.engine.getGameAttribute("L15NumSwTotalNum") : Float);
			_Item[2] = (((("" + Date.now().getHours()) + (":"))) + ((((("" + Date.now().getMinutes()) + (":"))) + ("" + Date.now().getSeconds()))));
			(Engine.engine.getGameAttribute("L15Date") : Array<Dynamic>)[Std.int((Engine.engine.getGameAttribute("L15NumSwTotalNum") : Float))] = _Item;
		}, null);
		...
		addMouseOverActorListener(_Match, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((_Matchable == true))
				{
					...
					_Target.disableActorDrawing();
					trace("True");
					trace((Engine.engine.getGameAttribute("L15Date") : Array<Dynamic>));
					if((_counter == 10))
					{
