...
class SceneEvents_45 extends SceneScript
{
	...
	public function _customEvent_ChooseTargetColorNew():Void
	{
		...
		_ATTLIST = (Engine.engine.getGameAttribute("L20Date") : Array<Dynamic>);
		_ACCT = randomInt(1, 2);
		if((_ACCT == 1))
		{
	...
	override public function init()
	{
		...
		Engine.engine.setGameAttribute("L20TotalNum", ((Engine.engine.getGameAttribute("L20TotalNum") : Float) + 1));
		runLater(1000 * .1, function(timeTask:TimedTask):Void
		{
			...
			_Item[1] = (Engine.engine.getGameAttribute("L20TotalNum") : Float);
			_Item[2] = (((("" + Date.now().getHours()) + (":"))) + ((((("" + Date.now().getMinutes()) + (":"))) + ("" + Date.now().getSeconds()))));
			(Engine.engine.getGameAttribute("L20Date") : Array<Dynamic>)[Std.int((Engine.engine.getGameAttribute("L20TotalNum") : Float))] = _Item;
		}, null);
		...
		addMouseOverActorListener(_Match, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((_Matchable == true))
				{
					...
					_Match.setX(-1200);
					trace("True");
					trace((Engine.engine.getGameAttribute("L20Date") : Array<Dynamic>));
					if((_counter == 10))
					{
