...
class SceneEvents_48 extends SceneScript
{
	...
	override public function init()
	{
		...
		Engine.engine.setGameAttribute("L14Num02TotalNum", (Engine.engine.getGameAttribute("L14Num02TotalNum") + 1));
		runLater(1000 * .1, function(timeTask:TimedTask):Void
		{
			...
			_Item[Std.int(1)] = Engine.engine.getGameAttribute("L14Num02TotalNum");
			_Item[Std.int(2)] = (("" + (("" + Date.now().getHours()) + ("" + ":"))) + ("" + (("" + (("" + Date.now().getMinutes()) + ("" + ":"))) + ("" + Date.now().getSeconds()))));
			Engine.engine.getGameAttribute("L14Date")[Std.int(Engine.engine.getGameAttribute("L14Num02TotalNum"))] = _Item;
		}, null);
		...
		addMouseOverActorListener(_Match, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((_Matchable == true))
				{
					...
					_Match.setX(-1200);
					trace("" + "True");
					trace("" + Engine.engine.getGameAttribute("L14Date"));
					if((_counter == 10))
					{
