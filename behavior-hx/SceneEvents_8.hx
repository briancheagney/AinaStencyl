...
class SceneEvents_8 extends SceneScript
{
	...
	override public function init()
	{
		...
		Engine.engine.setGameAttribute("L04TotalNum", (Engine.engine.getGameAttribute("L04TotalNum") + 1));
		runLater(1000 * .1, function(timeTask:TimedTask):Void
		{
			...
			_Item[Std.int(1)] = Engine.engine.getGameAttribute("L04TotalNum");
			_Item[Std.int(2)] = (("" + (("" + Date.now().getHours()) + ("" + ":"))) + ("" + (("" + (("" + Date.now().getMinutes()) + ("" + ":"))) + ("" + Date.now().getSeconds()))));
			Engine.engine.getGameAttribute("Lo4Date01")[Std.int(Engine.engine.getGameAttribute("L04TotalNum"))] = _Item;
		}, null);
		...
		_False = getActor(13);
		propertyChanged("_False", _False);
		
		/* ======================== When Creating ========================= */
		createRecycledActorOnLayer(getActorType(245), Engine.engine.getGameAttribute("TargetX"), Engine.engine.getGameAttribute("TargetY"), 0, "" + "2");
		_Target = getLastCreatedActor();
		propertyChanged("_Target", _Target);
		_Target.setAnimation("" + ("" + "NullClub"));
		createRecycledActorOnLayer(getActorType(245), Engine.engine.getGameAttribute("MatchX"), Engine.engine.getGameAttribute("MatchY"), 0, "" + "2");
		...
		_FalseLeewayBool = false;
		propertyChanged("_FalseLeewayBool", _FalseLeewayBool);
		_FalseLeeway = asNumber(0);
		...
		addMouseOverActorListener(_Target, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((Engine.engine.getGameAttribute("FalseLeewayBoolGA") == false))
				{
					Engine.engine.setGameAttribute("FalseLeewayBoolGA", true);
					runPeriodically(1000 * 1, function(timeTask:TimedTask):Void
					{
						Engine.engine.setGameAttribute("FalseLeewayGA", (Engine.engine.getGameAttribute("FalseLeewayGA") + 1));
						...
						if((Engine.engine.getGameAttribute("FalseLeewayGA") == 3))
						{
							Engine.engine.setGameAttribute("Level4Falser", true);
							timeTask.repeats = false;
							return;
						}
						if((Engine.engine.getGameAttribute("Level04Canceler") == true))
						{
							timeTask.repeats = false;
							return;
						}
		...
		addMouseOverActorListener(_Match, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				...
				Engine.engine.setGameAttribute("FalseLeewayBoolGA", false);
				Engine.engine.setGameAttribute("FalseLeewayGA", 0);
				if((Engine.engine.getGameAttribute("Level04Truable") == true))
				{
				...
				runLater(1000 * .25, function(timeTask:TimedTask):Void
				{
					...
					_tint = asNumber(0);
					propertyChanged("_tint", _tint);
					if((Engine.engine.getGameAttribute("Level04ActorAnim") == 1))
					{
						Engine.engine.setGameAttribute("Level04ActorAnim", 2);
					}
					else if((Engine.engine.getGameAttribute("Level04ActorAnim") == 2))
					{
						Engine.engine.setGameAttribute("Level04ActorAnim", 1);
					}
				}, null);
		...
		addMouseOverActorListener(getActor(13), function(mouseState:Int, list:Array<Dynamic>):Void
		{
			...
			{
				_Target.setAnimation("" + ("" + "NullClub"));
				if((Engine.engine.getGameAttribute("Level4Falser") == true))
				{
					if((_Match.isMouseOver() == false))
					{
						if((Engine.engine.getGameAttribute("TargetPush") == false))
						{
							Engine.engine.setGameAttribute("Level04Canceler", true);
							Engine.engine.setGameAttribute("FalseLeewayBoolGA", false);
							Engine.engine.setGameAttribute("FalseLeewayGA", 0);
