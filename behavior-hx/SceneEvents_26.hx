...
class SceneEvents_26 extends SceneScript
{
	...
	override public function init()
	{
		...
		Engine.engine.setGameAttribute("L10Col02TotalNum", (Engine.engine.getGameAttribute("L10Col02TotalNum") + 1));
		runLater(1000 * .1, function(timeTask:TimedTask):Void
		{
			...
			_Item[Std.int(1)] = Engine.engine.getGameAttribute("L10Col02TotalNum");
			_Item[Std.int(2)] = (("" + (("" + Date.now().getHours()) + ("" + ":"))) + ("" + (("" + (("" + Date.now().getMinutes()) + ("" + ":"))) + ("" + Date.now().getSeconds()))));
			Engine.engine.getGameAttribute("L10Date01")[Std.int(Engine.engine.getGameAttribute("L10Col02TotalNum"))] = _Item;
		}, null);
		...
		addMouseOverActorListener(_Target, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((Engine.engine.getGameAttribute("TargetPush") == true))
				{
					...
					/* In this example */
					_CardPicker = asNumber(randomInt(Math.floor(2), Math.floor(2)));
		...
		addMouseOverActorListener(_Match, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((_Matchable == true))
				{
					...
					_Match.setX(-1200);
					trace("" + "True");
					trace("" + Engine.engine.getGameAttribute("L10Date01"));
					if((_counter == 10))
					{
		...
		addMouseOverActorListener(_NoMatch, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((_Matchable == true))
				{
					...
					trace("" + "False");
					trace("" + Engine.engine.getGameAttribute("L09Date01"));
					if((_counter == 10))
					{
