...
class ActorEvents_0 extends ActorScript
{
	...
	override public function init()
	{
		...
		actor.setY(((getScreenHeight() / 2) - (((.01 * Engine.engine.getGameAttribute("SmallButtonSize")) * (actor.getHeight())) / 2)));
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				getActor(7).setX((((getScreenWidth() - 60) / 2) - (((.01 * Engine.engine.getGameAttribute("SmallButtonSize")) * ((getActor(7).getWidth()) / 2)) / 2)));
				actor.setY(((getScreenHeight() / 2) - (((.01 * Engine.engine.getGameAttribute("SmallButtonSize")) * (actor.getHeight())) / 2)));
			}
		});
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if((Engine.engine.getGameAttribute("Level02ButtonSize") == 3))
				{
					actor.setAnimation("" + _Large);
				}
				else if((Engine.engine.getGameAttribute("Level02ButtonSize") == 2))
				{
					actor.setAnimation("" + _Medium);
				}
				else if((Engine.engine.getGameAttribute("Level02ButtonSize") == 1))
				{
