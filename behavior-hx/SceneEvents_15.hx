...
class SceneEvents_15 extends SceneScript
{
	...
	override public function init()
	{
		...
		Engine.engine.setGameAttribute("L07TotalNum", (Engine.engine.getGameAttribute("L07TotalNum") + 1));
		runLater(1000 * .1, function(timeTask:TimedTask):Void
		{
			...
			_Item[Std.int(1)] = Engine.engine.getGameAttribute("L07TotalNum");
			_Item[Std.int(2)] = (("" + (("" + Date.now().getHours()) + ("" + ":"))) + ("" + (("" + (("" + Date.now().getMinutes()) + ("" + ":"))) + ("" + Date.now().getSeconds()))));
			Engine.engine.getGameAttribute("L07Date01")[Std.int(Engine.engine.getGameAttribute("L07TotalNum"))] = _Item;
		}, null);
		...
		addMouseOverActorListener(_Match, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((_Matchable == true))
				{
					...
					_Match.setX(-1200);
					trace("" + "True");
					trace("" + Engine.engine.getGameAttribute("L07Date01"));
					if((_counter == 10))
					{
