...
class Design_59_59_PushedLevel01 extends ActorScript
{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if((_Pressed == true))
				{
					_Pressed = false;
					propertyChanged("_Pressed", _Pressed);
					actor.growTo((.9 * _SizeW)/100, (.9 * _SizeH)/100, .25, Linear.easeNone);
					_Pressed = false;
					propertyChanged("_Pressed", _Pressed);
					runLater(1000 * .25, function(timeTask:TimedTask):Void
					{
						actor.growTo(100/100, 100/100, .1, Linear.easeNone);
					}, actor);
				}
			}
		});
